        ...
        <aside className="w-[360px] bg-surface-container border border-outline-variant flex flex-col rounded-sm shrink-0">
            <div className="p-3 border-b border-outline-variant flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-data-tabular font-data-tabular text-on-surface-variant text-xs">RX: LIVE</span>
                </div>
            </div>
            
            <div ref={scrollRef} className="flex-1 p-2 overflow-y-auto flex flex-col gap-2 bg-background">
                {packets.map(p => (
                    <div key={p.id} className={`bg-surface-container-high border p-2 flex flex-col gap-2 transition-colors ${getPacketStyle(p)}`}>
                        ...
                        {!p.acked && (
                            <div className="flex gap-2 mt-1">
                                {p.need_type === 'TABANG' && (
                                    <button className="flex-1 bg-primary-container text-black text-label-caps font-label-caps py-1 font-bold hover:bg-primary transition-colors">DISPATCH</button>
                                )}
                                <button onClick={() => ackPacket(p.id)} className="flex-1 bg-transparent border border-primary text-primary text-label-caps font-label-caps py-1 hover:bg-primary/10 transition-colors">ACK</button>
                            </div>
                        )}
                    </div>
                ))}
