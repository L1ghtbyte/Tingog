...
**Five buttons: TABANG (help), TUBIG (water), TAMBAL (medicine), PAGKAON (food), LUWAS (nothing urgent).**
...
- **LUWAS is the one worth real scrutiny.** It only works if someone actively presses it, and people reliably report problems, not confirm they're still fine. The fielded automatic heartbeat (§3) is what's supposed to cover this; this week's build doesn't have it. A genuinely fine community that doesn't press LUWAS looks identical to an unreachable one right now — an honest, named limitation of demo-week scope.
