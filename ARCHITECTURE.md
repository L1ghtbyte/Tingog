...
- **The fifth button is LUWAS, displayed as LUWAS.** LUWAS was originally assumed to translate to "safe"/"all-clear" — it doesn't; its actual root sense is closer to "to go out / to escape." That mismatch was caught during translation review, and the dashboard briefly displayed it as "OK" instead (an unambiguous replacement, chosen specifically so it couldn't be mistranslated) — reverted on 2026-08-17 back to showing the real button name. The wire protocol, backend `Event.button` values, and internal code have used `LUWAS` throughout regardless — that's the real hardware's wire code.
