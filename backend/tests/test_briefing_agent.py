...
def test_reconstruct_conversation_turns_replays_real_exchange_across_a_retry():
    ...
    # preceded it, and must not itself surface as if a coordinator typed it.
    messages = [
        {"role": "system", "content": "sys"},
        {"role": "user", "content": "What about Purok 4?"},
        {"role": "user", "content": "IMPORTANT — your previous attempt had a claim that didn't match..."},
        {"role": "assistant", "content": None, "tool_calls": [{"id": "1", "function": {"name": "get_purok", "arguments": "{}"}}]},
    ...
    assert turns == [
        {"question": "What about Purok 4?", "mode": "briefed", "narrative": "Purok 4 is stable.", "claims": [], "clarifying_question": None},
        {
            ...
            "clarifying_question": "Do you mean TUBIG or TAMBAL for Purok 5?",
        },
    ]
...
def test_reconstruct_conversation_turns_maps_default_general_text_to_null_question():
    ...
    assert turns == [{"question": None, "mode": "briefed", "narrative": "All quiet.", "claims": [], "clarifying_question": None}]
