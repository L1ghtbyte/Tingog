...
**On-demand — pre-record it, don't risk it live.**
- Measured during development: a real checked ("briefed") response ranged from ~3 seconds best case to 80+ seconds worst case, cycling through the provider fallback chain under real free-tier congestion. That variance is a real risk against a tight pitch clock.
- Record a genuine exchange ahead of time (a real call, real checked output, not staged) and play it back during the pitch. Honest framing if asked: "captured live during testing, playing it back so we don't spend pitch time waiting on a live AI call."
- Keep the recording ready as a fallback even if planning to attempt it live.
